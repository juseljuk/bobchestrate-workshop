...
| **Credentials via wxO Connections** | No hardcoded API keys — credentials are injected by wxO via `RunnableConfig`; read via `config.get("configurable", {}).get("credentials", {}).get("{app_id}_{credential_type}", "")` |
